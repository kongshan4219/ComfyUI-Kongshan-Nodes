...
class KSImageToTransparentBackground:
    """Apply mask(s) to an image and keep the foreground on a transparent background."""
    DESCRIPTION = "把输入图片中的遮罩区域保留为透明底 RGBA 图，适合生成裁切前的整图透明 PNG。"

    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                "image": ("IMAGE", {"tooltip": "原始或中间图片。输出会保持输入尺寸，只把遮罩外区域变透明。"}),
                "masks": ("MASK", {"tooltip": "一个或多个前景遮罩。多个遮罩会合并为一个 union mask，并作为 alpha 通道。"}),
            },
            "optional": {
                "edge_feather": (
                    "FLOAT",
                    {"default": 0.8, "min": 0.0, "max": 8.0, "step": 0.1, "tooltip": "alpha 边缘羽化半径。0 为硬边；数值越大边缘越柔和。"},
                ),
                "mask_threshold": (
                    "FLOAT",
    ...
    def process(
        ...
        source_array = np.clip(
            image[0].detach().cpu().numpy() * 255.0,
            0,
            255,
        ).astype(np.uint8)[..., :3]
        ...
        transparent_image = Image.fromarray(source_array, mode="RGB").convert("RGBA")
        transparent_image.putalpha(full_mask)
        result = torch.from_numpy(
            np.asarray(transparent_image).astype(np.float32) / 255.0
        )[None,]
...
NODE_CLASS_MAPPINGS = {
    "KSImageToTransparentBackground": KSImageToTransparentBackground,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "KSImageToTransparentBackground": "获取输入图片透明底图",
}
