...
from ._product_background_utils import _find_redundant_component_masks, _prepare_mask


class KSProductsToTransparentBackground:
    """Extract masked products and place them on transparent canvases."""
    DESCRIPTION = "把分割出的多个商品实例裁切、居中并铺到透明画布，适合生成带 alpha 通道的 PNG 商品图。"
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "masks": ("MASK", {"tooltip": "每个商品实例对应的遮罩批次。遮罩会作为透明 PNG 的 alpha 通道。"}),
                "output_size": (
                    "INT",
                    {"default": 1024, "min": 256, "max": 4096, "step": 64, "tooltip": "每个输出透明底图的正方形边长。值越大细节越多但文件更大、处理更慢。"},
                ),
                "margin_percent": (
                    "FLOAT",
                    {"default": 10.0, "min": 0.0, "max": 40.0, "step": 0.5, "tooltip": "商品四周透明留边比例。0 会尽量铺满画布；数值越大，商品越小、留边越多。"},
                ),
                "edge_feather": (
                    "FLOAT",
                    {"default": 0.8, "min": 0.0, "max": 8.0, "step": 0.1, "tooltip": "alpha 边缘羽化半径。0 保持硬边；数值越大边缘越柔和。"},
                ),
            ...
            "optional": {
                ...
                "fill_mask_holes": ("BOOLEAN", {"default": True, "tooltip": "是否填充遮罩内部空洞。开启可避免商品内部变透明；关闭可保留真实镂空区域。"}),
    ...
    RETURN_NAMES = (
        "transparent_background_products",
        "manifest",
        "product_count",
        "original_transparent_background",
    )
    RETURN_DESCRIPTIONS = (
        "逐个商品裁切、缩放、居中后的透明底 RGBA 图批次。",
        "JSON 清单，记录商品数量、输出序号、源实例索引和原图裁切框。",
        "最终输出的商品透明底图数量。",
        "把所有检测到的商品一起保留在原始画幅中的透明底整图。",
    )
    FUNCTION = "process"
    ...
    def process(
        ...
        full_rgba = Image.fromarray(source_array, mode="RGB").convert("RGBA")
        full_rgba.putalpha(full_mask)
        original_transparent_background = torch.from_numpy(
            np.asarray(full_rgba).astype(np.float32) / 255.0
        )[None,]
...
NODE_CLASS_MAPPINGS = {
    "KSProductsToTransparentBackground": KSProductsToTransparentBackground,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "KSProductsToTransparentBackground": "实例商品裁切为透明底图",
}
